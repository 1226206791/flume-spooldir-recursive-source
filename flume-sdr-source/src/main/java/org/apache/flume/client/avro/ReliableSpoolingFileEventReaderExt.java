...
import org.apache.flume.source.SpoolDirRecursiveSourceConfigurationConstants;
...
public class ReliableSpoolingFileEventReaderExt implements ReliableEventReader {

    private static final Logger logger = LoggerFactory
            .getLogger(org.apache.flume.client.avro.ReliableSpoolingFileEventReader.class);
    ...
    private final long afterModified;
     ...
     */
    private ReliableSpoolingFileEventReaderExt(File spoolDirectory,
                                               ...
                                               ConsumeOrder consumeOrder, long afterModified) throws IOException {
        ...
        if (logger.isDebugEnabled()) {
            logger.debug("Initializing {} with directory={}, metaDir={}, " +
                            "deserializer={}",
                    new Object[]{org.apache.flume.client.avro.ReliableSpoolingFileEventReader.class.getSimpleName(),
...
//        List<File> candidateFiles = Collections.EMPTY_LIST;
            ...
            FileFilter filter = new FileFilter() {
                public boolean accept(File candidate) {
                    ...
                    if ((fileName.endsWith(completedSuffix)) ||
                            ...
                            System.currentTimeMillis() - candidate.lastModified() < ReliableSpoolingFileEventReaderExt.this.afterModified) {
                        return false;
                    }
                    return true;
                }
...
//            candidateFiles = Arrays.asList(spoolDirectory.listFiles(filter));
    ...
    private void listDirFiles(List<File> files, File dir, FileFilter filter) {
        File[] children = dir.listFiles(filter);// 列出目录下所有的文件
        for (int i = 0; i < children.length; i++) {
            if (children[i].isFile()) {
                files.add(children[i]);
            } else {
                if (children[i].isDirectory()) {
                    listDirFiles(files, children[i], filter);// 如果是个目录,就继续递归列出其下面的文件.
                }
            }
        }
    }
    ...
    public static class Builder {
        ...
        private long afterModified = SpoolDirRecursiveSourceConfigurationConstants.DEFAULT_AFTER_MODIFY;

        public Builder afterModified(long times) {
            this.afterModified = times;
            return this;
        }
        ...
        public ReliableSpoolingFileEventReaderExt build() throws IOException {
            return new ReliableSpoolingFileEventReaderExt(spoolDirectory, completedSuffix,
